...
export default function BreathingExercise({
  ...
  if (colorProgress < 0.33) {
    ...
    currentColor = interpolateColor(
      colors[1],
      colors[2],
      (colorProgress - 0.33) / 0.33,
    );
  } else {
    currentColor = interpolateColor(
      colors[2],
      colors[3],
      (colorProgress - 0.66) / 0.34,
    );
  }
                ...
                <div className="mt-8">
                  <div className="w-32 h-32 mx-auto bg-gray-100 rounded-full flex items-center justify-center">
                    <span className="text-4xl font-bold text-gray-700 tabular-nums">
                      {Math.floor(workSecondsLeft / 60)}:
                      {String(workSecondsLeft % 60).padStart(2, "0")}
                    </span>
                  </div>
          ...
          <div className="fixed bottom-12 left-1/2 transform -translate-x-1/2 w-80">
            <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: sessionStartTime
                    ? `${Math.min(100, ((currentTime - sessionStartTime) / 1000 / TOTAL_SESSION_TIME) * 100)}%`
                    : "0%",
                }}
              />
            </div>
